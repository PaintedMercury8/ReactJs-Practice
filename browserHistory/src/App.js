...
class App extends Component {
  ...
  changeHistoryList = id => {
    ...
    const changedArray = historyList.filter(eachItem => eachItem.id !== id)
    this.setState({historyList: changedArray})
  }

  render() {
    const {searchText, historyList} = this.state
    console.log(searchText, historyList)
    const lowerCase = searchText.toLowerCase()
    const searchHistoryList = historyList.filter(
      everyItem => everyItem.title.toLowerCase().includes(lowerCase) === true,
    )
    return (
