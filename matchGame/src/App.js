...
import DisplayItems from './Components/DisplayItems/index'
...
class App extends Component {
  state = {
    ...
    matchImg: imagesList[0].imageUrl,
    matchId: imagesList[0].id,
    hasTimeOutOrLost: false,
  }
  ...
  componentDidMount() {
    this.timerID = setInterval(() => {
      ...
      if (seconds === 1) {
        ...
        this.setState({hasTimeOutOrLost: true})
      }
    }, 1000)
  }
  ...
  checkMatch = uid => {
    const {matchId, hasTimeOutOrLost} = this.state
    if (matchId === uid) {
      const randomUrl =
        imagesList[Math.floor(Math.random() * imagesList.length)]
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchImg: randomUrl.imageUrl,
        matchId: randomUrl.id,
      }))
    } else {
      this.setState({hasTimeOutOrLost: true})
      clearInterval(this.timerID)
  ...
  mainContent = () => {
    const {score, seconds, matchImg, category, hasTimeOutOrLost} = this.state
    const filterList = imagesList.filter(
      eachItem => eachItem.category === category,
    )
    return (
      <>
        <img src={matchImg} alt="match" className="match-img" />
        ...
        <header className="nav-bar">
          <ul className="header-list">
            ...
            <li className="score-timer-container">
              <p className="score">
                Score:<span className="score-display-top">{score}</span>
              </p>
              <div className="timer-container">
                <img
                  ...
                  className="timer-img"
                />
                <p className="timer-count">{seconds} sec</p>
              </div>
            </li>
          </ul>
        </header>
        <div className="main-container">
          {hasTimeOutOrLost === false ? this.mainContent() : this.lostScreen()}
        </div>
      </div>
    )
  }
